...
public static unsafe class BehaviourState
{
    ...
    public static ulong Capture(ulong handle)
    {
            ...
            foreach (var field in CapturableFields(type))
            {
                ...
                else
                {
                    Console.Error.WriteLine(
                        $"[Ferron] {type.Name}.{field.Name}: {field.FieldType.Name} is not "
        ...
        {
            Console.Error.WriteLine($"[Ferron] capture failed: {e}");
            return 0;
        }
    }
    ...
    public static byte Apply(ulong handle, ulong id)
    {
        try
        {
            ...
            foreach (var field in CapturableFields(type))
            {
                ...
                {
                    Console.Error.WriteLine(
                        $"[Ferron] {type.Name}.{field.Name} changed shape across the reload; "
        ...
        {
            Console.Error.WriteLine($"[Ferron] apply failed: {e}");
            return 0;
        }
    }
    ...
    static Dictionary<string, object?>? DefaultsOf(Type type)
    {
        ...
        catch (Exception e)
        {
            ...
            Console.Error.WriteLine(
                $"[Ferron] could not read {type.Name}'s defaults ({e.GetType().Name}); "
    ...
    static bool TryCapture(FieldInfo field, object instance, out object? value)
    {
        ...
        // Ferron's value types stay plain data: one that grew a reference field
