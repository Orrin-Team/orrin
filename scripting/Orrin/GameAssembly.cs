...
///
/// `Orrin.dll` itself is never loaded here. It is the ABI surface — Rust holds
...
/// `load_assembly_and_get_function_pointer` puts `Orrin.dll` in an isolated
...
internal sealed class GameLoadContext : AssemblyLoadContext
{
    ...
    internal GameLoadContext(string directory)
        : base(name: "OrrinGame", isCollectible: true) => _directory = directory;

    protected override Assembly? Load(AssemblyName name)
    {
        if (name.Name is null)
            return null;
        if (name.Name == OrrinAssemblyName)
            return Bindings;
    ...
    internal static readonly string OrrinAssemblyName = Bindings.GetName().Name!;
...
public static unsafe class GameAssembly
{
    ...
    public static int Load(byte* dllPath)
    {
        ...
        {
            Console.Error.WriteLine($"[Orrin] failed to load game assembly {path}: {e}");
            _pending = null;
            return LoadFailed;
        }
    }
