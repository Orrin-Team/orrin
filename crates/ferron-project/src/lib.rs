...
"#;
    ...
    /// Write `body` as `ferron.toml` in a fresh temp dir and hand back both the
    ...
    fn project_dir(body: &str) -> (TempDir, PathBuf) {
        ...
        std::fs::write(dir.path().join("ferron.toml"), body).expect("write manifest");
    ...
    fn round_trip() {
        ...
        assert_eq!(project.name(), "hello-ferron");
    ...
    fn locate_returns_none_without_a_manifest() {
        ...
        // just means "not a Ferron project".
        assert!(
            Project::locate(&root)
    ...
    fn load_reports_a_missing_file() {
        let dir = tempfile::tempdir().expect("tempdir");
        let missing = dir.path().join("ferron.toml");
