...
pub fn sky_equirect(width: u32, height: u32, to_sun: Vec3) -> Vec<f32> {
    ...
    // Scaled so the irradiance this integrates to lands near the flat ambient
    // the demo used before it had an environment. Physical sky radiance is
    // several times higher; matching it is what auto-exposure is for, and until
    // that exists a placeholder that blows out the tonemap on first launch is
    // just a worse first impression.
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            // orders of magnitude above the sky even after the scaling above:
            // it is what a smooth metal reflects and what the prefilter's
            // firefly handling exists to survive, so flattening it would hide
