...
//! Run with the cwd set to the built `Orrin.dll` directory
//! (`scripting/Orrin/bin/Debug/net10.0`).
//!
//! With no arguments it drives a probe that lives in the bindings assembly, so
//! it needs nothing but `dotnet build scripting/Orrin`. Pass a game DLL and a
//! type name to exercise the collectible-load-context path instead:
//!
//! ```text
//! cargo run -p orrin-script --example spike -- \
...
fn main() {
    ...
    let type_name = match (args.next(), args.next()) {
        ...
        _ => "Orrin.Tests.LifecycleProbe, Orrin".to_owned(),
    };
