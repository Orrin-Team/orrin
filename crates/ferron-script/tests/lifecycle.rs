...
use ferron_script::{CEntity, GameAssemblyStatus, ScriptHost};
...
fn game_assembly() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("FERRON_GAME_DLL") {
...
fn state_preservation(host: &ScriptHost) {
    let probe = "Ferron.Tests.StatefulProbe, Ferron";
    ...
    assert_ne!(untouched, 0, "StatefulProbe not found in Ferron.dll");
    assert_eq!(
        host.capture_state(untouched),
        0,
        "a behaviour still at its constructor values has nothing worth preserving"
    );
    ferron_script::destroy_handle(untouched);
    take_logs();

    let handle = create(host, probe);
    assert_ne!(handle, 0, "StatefulProbe not found in Ferron.dll");
    host.enable(handle);
    host.update(handle, 0.016);
    host.update(handle, 0.016);
    let logs = take_logs();
    ...
    assert!(state.contains("offset=3"), "Offset (a Ferron value type) must survive: {state}");
...
/// (sharing the *one* Ferron.dll in the default context), and once every handle
...
fn game_assembly_swap(host: &ScriptHost) {
    ...
    // worked; the behaviour running proves it bound to the *same* Ferron.dll
    ...
    ferron_script::destroy_handle(first);
    assert_eq!(host.stage_game(&path), GameAssemblyStatus::Ok);
    assert_eq!(
        host.commit_game(),
        GameAssemblyStatus::Ok,
    ...
    ferron_script::destroy_handle(second);
